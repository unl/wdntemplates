...
h=0;setTimeout(function(){a("#play_pause_btn"+b).addClass("play")},c.transitionSpeed);a("#play_pause_btn"+b).unbind("click").bind("click",function(){s()})}if(target_num[1]>f[0]){diff=target_num[1]-f[0];q(diff)}if(target_num[1]<f[0]){diff=f[0]-target_num[1];G(diff)}}var f=[],r=[];a(d).after('<div id="thumbs'+b+'" class="thumb-wrapper" />');for(i=0;i<l;i++){thumb=a("img:eq("+i+")",d).attr("src");j=a('<div class="thumb" id="thumb'+b+"_"+(i+1)+'" />');j.hover(function(){var e=a("#thumbs"+b+" div.thumb.active").css("opacity")||
1;a(this).animate({opacity:e},150)},function(){var e=a("#thumbs"+b+" div.thumb:not(.active)").css("opacity")||0.65;a.inArray(this.id.split("_")[1],f)>=0&&a(this).animate({opacity:e},250)}).bind("click",K);thumb&&c.displayThumbnailBackground&&j.css({"background-image":"url("+thumb+")"});c.displayThumbnailNumbers&&j.text(i+1);a("#thumbs"+b).append(j);i<=c.inView&&a("#thumb"+b+"_"+i).addClass("active");r.push(i+1)}for(i=1;i<=c.inView;i++)f.push(r.shift())}a("li:last",d).prependTo(a("ul",d));a("ul",d).css("left",
-g+"px");if(c.showControls){a('<div class="playpause" id="play_pause_btn'+b+'" />').appendTo(d);var p="play",A=a("#play_pause_btn"+b).css("opacity")||0.5;a("#play_pause_btn"+b).hover(function(){a(this).animate({opacity:"1"},250)},function(){a(this).animate({opacity:A},250)});a("#play_pause_btn"+b).click(function(){p=p=="play"?"pause":"play";p=="play"?s():z()});if(!c.prevNextInternal){wrapID=a(d).attr("id")+"Wrapper";a(d).wrap('<div id="'+wrapID+'"></div>').css("margin","0 auto");a("#"+wrapID).css("position",
"relative").width(a(d).width()+40+parseInt(a(d).css("padding-left"))+parseInt(a(d).css("padding-right")))}var u=n/2-15;a('<div id="btn_rt'+b+'" class="btn_rt" />').css({top:u+"px"}).appendTo(c.prevNextInternal?d:a("#"+wrapID));a('<div id="btn_lt'+b+'" class="btn_lt" />').css({top:u+"px"}).appendTo(c.prevNextInternal?d:a("#"+wrapID));var L=a("#btn_rt"+b).css("opacity")||0.5;a("#btn_rt"+b).click(function(){o("next")}).hover(function(){a(this).animate({opacity:"1"},250)},function(){a(this).animate({opacity:L},
250)});a("#btn_lt"+b).click(function(){o("prev")}).hover(function(){a(this).animate({opacity:"1"},250)},function(){a(this).animate({opacity:L},250)});if(c.autoHideControls&&c.prevNextInternal){function N(){a("#play_pause_btn"+b).stop().animate({top:"3px",right:"3px"},250);a("#btn_rt"+b).stop().animate({top:u+"px",right:"2px"},250);a("#btn_lt"+b).stop().animate({top:u+"px",left:"2px"},250)}function M(){a("#play_pause_btn"+b).stop().animate({top:-16-k+"px",right:-16-k+"px"},250);a("#btn_rt"+b).stop().animate({right:"-16px"},
250);a("#btn_lt"+b).stop().animate({left:"-16px"},250)}a(d).hover(N,M);M()}if(c.autoHideCaptions){var B;function J(){B=true;for(i=1;i<=c.inView;i++)w(a("li:eq("+i+") p",d).html(),i)}function O(){B=false;y()}a(d).hover(J,O);y()}}if(h){var t=setInterval(function(){q()},c.displayTime+c.transitionSpeed);c.displayProgressBar&&D(c.displayTime+c.transitionSpeed)}else{p="pause";a("#play_pause_btn"+b).addClass("play")}F()})}})})(WDN.jQuery);
