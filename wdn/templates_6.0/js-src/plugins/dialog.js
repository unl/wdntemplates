import dialogs_css_url from '@scss/components-js/_dialogs.scss?url';
...
const query_selector = ".dcf-dialog";
...
export async function initialize() {
    ...
    const notice_component = await import('@js-src/components/wdn_dialog.js');
    wdn_dialog = notice_component.default;
    await loadStyleSheet(dialogs_css_url);
}
...
export async function load_element(element, options) {
    ...
    return new wdn_dialog(element, options);
}
...
export async function load_elements_on_page(options) {
    let all_dialogs = document.querySelectorAll(query_selector);
    return await load_elements(all_dialogs, options);
}
